...
public class Doc_AllocationHdr extends Doc
{
	...
	{
		isMerge = MSysConfig.getBooleanValue("Is_Allocation_Merge_Account", false, getAD_Client_ID());
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			else if (invoice.isSOTrx())
			{
				...
				if (as.isAccrual())
				{
					...
					flForRGL.setUserElement1_ID(invoice.getC_Invoice_ID());
				}
				else	//	Cash Based
				{
...
//					payment currency
		...
		//
		m_facts.add(fact);
		if(MSysConfig.getBooleanValue("Is_Allocation_Merge_Account", false, getAD_Client_ID())) 
		m_facts = mergeAccount(m_facts);
		...
		BigDecimal allocationSource)
	{
		...
		if (as.isCreatePOCommitment() && !invoice.isSOTrx())
		{
			...
			for (int i = 0; i < lines.length; i++)
			{
				Fact factC = Doc_Order.getCommitmentReleasePublic(as, this,
		...
		MInvoice invoice, BigDecimal allocationSource, BigDecimal allocationAccounted)
	{
		...
		if (alloc.getReversal_ID() == 0 || alloc.get_ID() < alloc.getReversal_ID())
		{
			if (hasDebitTradeAmt(invoice))
			{
				...
				//2023-09-04 瑋君提出兌換損益的應收帳款-固定列在 Credit
				//fl = fact.createLine (line, acct, as.getC_Currency_ID(), acctDifference.negate());
				invGainLossFactLines.add(fl);
				fl = fact.createLine (line, acct, as.getC_Currency_ID(), null,acctDifference);
				fl.setUserElement1_ID(invoice.getC_Invoice_ID());
				fl.setDescription(description.toString());
				if(isMerge)
				{
			...
			{
				//2023-09-07 瑋君提出兌換損期的應付帳款-固定列在 Debit
				//FactLine fl = fact.createLine (line, acct, as.getC_Currency_ID(), acctDifference);
				FactLine fl = fact.createLine (line, acct, as.getC_Currency_ID(),acctDifference, null);
				fl.setUserElement1_ID(invoice.getC_Invoice_ID());
				fl.setDescription(description.toString());
				if(isMerge)
				{
	...
	private ArrayList<FactLine> mergeFactLinesAccount(ArrayList<FactLine> tmpFactLines) {

		ArrayList<FactLine> newLines  =   new ArrayList<FactLine>();		    
	    for (FactLine fl : tmpFactLines) {
			if(!accumulateToExistingFactLine(fl,newLines))
			{
				newLines.add(fl);
			}
		}
		...
		MPayment payment, BigDecimal allocationSource, BigDecimal allocationAccounted)
	{
		...
		{
			// 2023-09-19 付款差異 (強制放在貸方)
			//FactLine fl = fact.createLine (line, acct, as.getC_Currency_ID(), acctDifference);
			FactLine fl = fact.createLine (line, acct, as.getC_Currency_ID(), null,acctDifference.negate());
			if(isMerge)
			{
				fl.setAmtSourceDr(BigDecimal.ZERO);
				fl.setAmtSourceCr(BigDecimal.ZERO);
	...
	{	
		...
		for (MPayment payment : payList)
		{
			...
			if ((payment.isReceipt() && payment.getPayAmt().signum() >= 0) || (!payment.isReceipt() && payment.getPayAmt().signum() < 0))
			{
				// 21300_Payment selection 強制在借方
				// 2023-10-05
				//FactLine fl = fact.createLine (null, htPayAcct.get(payment.getC_Payment_ID()), as.getC_Currency_ID(), acctDifference.negate());
				FactLine fl = fact.createLine (null, htPayAcct.get(payment.getC_Payment_ID()), as.getC_Currency_ID(), acctDifference.negate(),null);
				fl.setUserElement1_ID(0);
				fl.setDescription(description.toString());
				fl.setLine_ID(C_AllocationLine_ID == null ? 0 : C_AllocationLine_ID);
				if(isMerge)
	...
	private ArrayList<Fact> mergeAccount(ArrayList<Fact> facts) {
		ArrayList<Fact>  newFacts = new ArrayList<Fact>();
		for (Fact fact : facts) {
			Fact newFact  = new Fact(this, fact.getAcctSchema(), Fact.POST_Actual);
			
			FactLine[] lines  =  fact.getLines();			    
			    for (FactLine fl : lines) {
					if(!accumulateToExistingFactLine(fl,newFact))
					{
						newFact.add(fl);
					}
				}
	...
	private boolean accumulateToExistingFactLine(FactLine fl, ArrayList<FactLine> lines ) {
	
		for (FactLine factLine : lines) {
			if(factLine.getAccount_ID() == fl.getAccount_ID())
			{
				factLine.setAmtAcctDr(factLine.getAmtAcctDr().add(fl.getAmtAcctDr()));
				factLine.setAmtAcctCr(factLine.getAmtAcctCr().add(fl.getAmtAcctCr()));
				factLine.setAmtSourceDr(factLine.getAmtSourceDr().add(fl.getAmtSourceDr()));
				factLine.setAmtSourceCr(factLine.getAmtSourceCr().add(fl.getAmtSourceCr()));
				factLine.setQty(fl.getQty());
				return true;
			}
		}
	...
	private boolean accumulateToExistingFactLine(FactLine fl, Fact newFact) {
		FactLine[] lines  =  newFact.getLines();		
		for (FactLine factLine : lines) {
//			System.out.println("factLine.getUserElement1_ID()" + factLine.getUserElement1_ID());
//			System.out.println("fl.getUserElement1_ID()" + fl.getUserElement1_ID());

			if(factLine.getAccount_ID() == fl.getAccount_ID() && factLine.getUserElement1_ID()  == fl.getUserElement1_ID())
			{
				factLine.setAmtAcctDr(factLine.getAmtAcctDr().add(fl.getAmtAcctDr()));
				factLine.setAmtAcctCr(factLine.getAmtAcctCr().add(fl.getAmtAcctCr()));
				factLine.setAmtSourceDr(factLine.getAmtSourceDr().add(fl.getAmtSourceDr()));
